  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      if (url.pathname === "/api/cms/bootstrap") {
        ...
        if (!sanityProjectId || !CMS_CONFIGURED.has(projectId) || sanityProjectId.startsWith("PLACEHOLDER")) {
      ...
      if (url.pathname === "/api/forms/submit") {
        ...
        await env.DB.prepare(
          ...
          leadId, "contact", name, email, phone, String(body.message || ""), JSON.stringify({ serviceType: body.serviceType || body.service || null }),
