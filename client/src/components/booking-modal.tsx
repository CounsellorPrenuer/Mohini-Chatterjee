...
export default function BookingModal({ open, onOpenChange, planId, title, category, price }: Props) {
  ...
  const openMailDraft = () => {
    const subject = encodeURIComponent(`Aakaar booking enquiry: ${title}`);
  ...
  const pay = async () => {
    ...
    try {
      ...
      const checkout = new window.Razorpay({
        ...
        name: "Aakaar - Mohini Chatterjee",
        description: title,
        order_id: order.order_id,
        prefill: { name, email, contact: phone },
        theme: { color: "hsl(221, 83%, 53%)" },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[92vh] overflow-y-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-3 pt-2">
          ...
          <Button onClick={pay} disabled={isProcessing} className="gradient-bg">
